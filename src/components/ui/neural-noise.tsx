...
export function NeuralNoise({ color = [1.0, 0.384, 0.169], opacity = 0.95, speed = 0.001 }) {
  ...
  useEffect(() => {
    ...
    if (uniforms && uniforms.u_color) {
      gl.uniform3f(uniforms.u_color, color[0], color[1], color[2]);
    }
    if (uniforms && uniforms.u_speed) {
  ...
  function setupEvents() {
    ...
    window.addEventListener('touchmove', touchmove as any);
    window.addEventListener('click', click as any);
    return () => {
      window.removeEventListener('pointermove', pointermove as any);
      window.removeEventListener('touchmove', touchmove as any);
      window.removeEventListener('click', click as any);
    };
  }
